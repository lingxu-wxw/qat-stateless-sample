...
out:
	...
	int queue_page_request_list(struct zram *zram, struct bio_request *bio_req,
								struct bio_vec *bvec, u32 index, int offset,bool write, struct list_head *page_list)
	{
	struct page_request *page_req = kmalloc(sizeof(*page_req), GFP_NOIO);

	if (!page_req) {
		while (!list_empty(page_list)) {
			page_req = list_first_entry(page_list,struct page_request, list);
...
static int __zram_cpu_notifier(void *dummy, unsigned long action, unsigned long cpu)
{
